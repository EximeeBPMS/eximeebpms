...
public class GroovySecureAstCustomizerFactory implements ScriptEngineResolver {

  protected final ScriptEngineManager scriptEngineManager;

  protected Map<String, ScriptEngine> cachedEngines = new HashMap<>();

  public GroovySecureAstCustomizerFactory(ScriptEngineManager scriptEngineManager) {
  ...
  protected ScriptEngine getScriptEngine(String language) {
    ...
    } else if (ScriptingEngines.GROOVY_SCRIPTING_LANGUAGE.equalsIgnoreCase(language)) {
      scriptEngine = getGroovyScriptEngine();
    } else {
