 ...
 */
package org.eximeebpms.bpm.run.qa;
...
public class ComponentAvailabilityIT {
  ...
  public void shouldFindWelcomeApp() {
    Response response = when().get(container.getBaseUrl() + "/eximeebpms/app/welcome/default");
    if (webappsAvailable) {
      response.then()
        .statusCode(200)
        .body("html.head.title", equalTo("EximeeBPMS Welcome"));
    } else {
      response.then()
        .statusCode(404);
    }
