...
const addMissingLicenseHeaders = (filePath, source) => {
  ...
  // See https://github.com/camunda/eximeebpms-bpm-platform/issues/2824
  ...
  if (
    ...
    } else if (!rowFile.includes('eximeebpms-bpm-sdk-js')) {
      pkg = rowFile.replace(
        /^(@[a-z-\d.]+\/[a-z-\d.]+)?([a-z-\d.]+)?(.*)$/,
        (match, p1, p2) => p2 || p1,
      );
    ...
    if (pkg) {
      ...
      if (!fs.existsSync(packagePath)) {
        packagePath = `${process.cwd()}/node_modules/eximeebpms-bpm-webapp/node_modules/${pkg}`;
      }
