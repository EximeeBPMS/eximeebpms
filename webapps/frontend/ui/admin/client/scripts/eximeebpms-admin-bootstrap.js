...
window.bust = EXIMEEBPMS_VERSION; // eslint-disable-line
window.DOMPurify = require('dompurify');

// eximeebpms-admin-bootstrap is copied as-is, so we have to inline everything
...
requirejs.config({
  ...
  urlArgs: `bust=${EXIMEEBPMS_VERSION}` // eslint-disable-line
});
...
define('eximeebpms-admin-bootstrap', function () {
  'use strict';
  const bootstrap = (config) => {
    ...
    var camundaAdminUi = require('./eximeebpms-admin-ui');
    camundaAdminUi.exposePackages(window);

    requirejs([`${appRoot}/lib/globalize.js`], function (globalize) {
      globalize(
        requirejs,
        ['angular', 'eximeebpms-commons-ui', 'eximeebpms-bpm-sdk-js', 'jquery'],
        window,
      );
      ...
      pluginPackages.forEach(function (plugin) {
        ...
        node.setAttribute(
          'href',
          plugin.location + `/plugin.css?bust=${EXIMEEBPMS_VERSION}` // eslint-disable-line
        );
...
requirejs(['eximeebpms-admin-bootstrap'], function () {});
