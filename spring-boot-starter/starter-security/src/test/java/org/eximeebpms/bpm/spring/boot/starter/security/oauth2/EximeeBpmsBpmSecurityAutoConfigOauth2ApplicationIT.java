...
import org.eximeebpms.bpm.spring.boot.starter.security.oauth2.impl.AuthorizeTokenFilter;
import org.eximeebpms.bpm.spring.boot.starter.security.oauth2.impl.OAuth2AuthenticationProvider;
...
public class EximeeBpmsBpmSecurityAutoConfigOauth2ApplicationIT extends AbstractSpringSecurityIT {
  ...
  public void testSpringSecurityAutoConfigurationCorrectlySet() {
    ...
    assertThat(getBeanForClass(EximeeBpmsSpringSecurityOAuth2AutoConfiguration.class, mockMvc.getDispatcherServlet().getWebApplicationContext())).isNotNull();
    assertThat(getBeanForClass(EximeeBpmsBpmSpringSecurityDisableAutoConfiguration.class, mockMvc.getDispatcherServlet().getWebApplicationContext())).isNull();
  }
  ...
  public void testWebappWithoutAuthentication() throws Exception {
    ...
    // when
    mockMvc.perform(MockMvcRequestBuilders.get(baseUrl + "/eximeebpms/api/engine/engine/default/user")
