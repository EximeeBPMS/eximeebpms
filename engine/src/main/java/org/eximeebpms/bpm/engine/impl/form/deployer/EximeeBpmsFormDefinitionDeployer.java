 ...
 */
package org.eximeebpms.bpm.engine.impl.form.deployer;
...
public class EximeeBpmsFormDefinitionDeployer extends AbstractDefinitionDeployer<EximeeBpmsFormDefinitionEntity> {
  ...
  @Override
  protected List<EximeeBpmsFormDefinitionEntity> transformDefinitions(DeploymentEntity deployment, ResourceEntity resource,
    ...
    try {
      ...
      EximeeBpmsFormDefinitionEntity definition = new EximeeBpmsFormDefinitionEntity(camundaFormDefinitionKey, deployment.getId(), resource.getName(), deployment.getTenantId());
  ...
  @Override
  protected EximeeBpmsFormDefinitionEntity findDefinitionByDeploymentAndKey(String deploymentId, String definitionKey) {
    return getCommandContext().getEximeeBpmsFormDefinitionManager().findDefinitionByDeploymentAndKey(deploymentId,
        definitionKey);
  }

  @Override
  protected EximeeBpmsFormDefinitionEntity findLatestDefinitionByKeyAndTenantId(String definitionKey, String tenantId) {
    return getCommandContext().getEximeeBpmsFormDefinitionManager().findLatestDefinitionByKeyAndTenantId(definitionKey,
        tenantId);
  }

  @Override
  protected void persistDefinition(EximeeBpmsFormDefinitionEntity definition) {
    getCommandContext().getEximeeBpmsFormDefinitionManager().insert(definition);
  }

  @Override
  protected void addDefinitionToDeploymentCache(DeploymentCache deploymentCache,
      EximeeBpmsFormDefinitionEntity definition) {
    deploymentCache.addEximeeBpmsFormDefinition(definition);
  }
