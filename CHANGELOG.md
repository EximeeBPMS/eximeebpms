...
- Replaced the deprecated MSSQL `image` data type with `varbinary(max)` for `ACT_ID_INFO.PASSWORD_`, `ACT_GE_BYTEARRAY.BYTES_`, and `ACT_HI_COMMENT.FULL_MSG_` (carried in the `1.3-to-1.4` upgrade script) — Microsoft has deprecated `image`/`text`/`ntext` and plans to remove them in a future SQL Server version. New installs get `varbinary(max)` directly via the updated create/Liquibase-baseline scripts. **Migration:** nothing to plan for. `image` and `varbinary(max)` share the same LOB storage, so SQL Server applies this as a metadata-only change — measured on a 1 GB `ACT_GE_BYTEARRAY` (4,096 rows × 256 KiB, 139,273 LOB pages; SQL Server 2022, FULL recovery with an open log chain): 4 ms, not one LOB page rewritten, 60 KB of transaction log. Like any `ALTER TABLE` it still takes a brief schema-modification lock, so it waits for transactions open on the table to finish — but there is no data movement, and no maintenance window is needed.
