# eximeebpms-bpm-spring-boot-starter

[cols="a,a,a,a"]
,====
// mvn travis
image::https://travis-ci.org/eximeebpms/eximeebpms-bpm-spring-boot-starter.svg?branch=master[link="https://travis-ci.org/eximeebpms/eximeebpms-bpm-spring-boot-starter"]
// license
image::https://img.shields.io/badge/License-Apache%202.0-blue.svg[link="./LICENSE"]
// mvn central
image::https://maven-badges.herokuapp.com/maven-central/org.eximeebpms.bpm.springboot/eximeebpms-bpm-spring-boot-starter/badge.svg[link="https://maven-badges.herokuapp.com/maven-central/org.eximeebpms.bpm.springboot/eximeebpms-bpm-spring-boot-starter"]
// openhub
image::https://www.openhub.net/p/eximeebpms-bpm-spring-boot-starter/widgets/project_thin_badge.gif[link="https://www.openhub.net/p/eximeebpms-bpm-spring-boot-starter"]
,====

This project provides Spring Boot starters that allow you to integrate the EximeeBPMS into your Spring Boot applications.

Please check out the https://docs.eximeebpms.org/manual/latest/user-guide/spring-boot-integration/[Documentation],
the https://docs.eximeebpms.org/get-started/spring-boot/[Getting Started Guide] and
...
[IMPORTANT]
We have decided to refactor the configuration mechanism. Especially, we removed the  `org.eximeebpms.bpm.spring.boot.starter.configuration.CamundaConfiguration` interface. For now, each configuration is a `org.eximeebpms.bpm.engine.impl.cfg.ProcessEnginePlugin`. With this it is possible to hook into `preInit`, `postInit` and `postProcessEngineBuild`, which supports more complex situations.
If you have implemented a `CamundaConfiguration`, you have to switch to `ProcessEnginePlugin` (and its `preInit` method). You can also extend your configuration from `org.eximeebpms.bpm.spring.boot.starter.configuration.impl.AbstractCamundaConfiguration` to avoid a cast to `SpringProcessEngineConfiguration`.
