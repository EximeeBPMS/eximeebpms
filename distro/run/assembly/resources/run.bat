...
IF [%detachProcess%]==[true] (
  ...
  start "%APPNAME%" "%JAVA%" -Dloader.path="%classPath%" -Deximeebpms.deploymentDir="%DEPLOYMENTDIR%" %JAVA_OPTS% -jar "%BASEDIR%eximeebpms-bpm-run-core.jar" --spring.config.location=file:"%configuration%"

) ELSE (
  call "%JAVA%" -Dloader.path="%classPath%" -Deximeebpms.deploymentDir="%DEPLOYMENTDIR%" %JAVA_OPTS% -jar "%BASEDIR%eximeebpms-bpm-run-core.jar" --spring.config.location=file:"%configuration%"
)
