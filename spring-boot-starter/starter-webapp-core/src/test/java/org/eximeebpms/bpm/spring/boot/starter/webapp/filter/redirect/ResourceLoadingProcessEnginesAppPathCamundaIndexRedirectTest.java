 ...
 */
package org.eximeebpms.bpm.spring.boot.starter.webapp.filter.redirect;
...
public class ResourceLoadingProcessEnginesAppPathCamundaIndexRedirectTest {
  ...
  public void shouldRedirectRequestToTasklist_contextRoot() {
    // when
    // send GET request to /eximeebpms
    HttpURLConnection con = rule.performRequest("http://localhost:" + port + "/eximeebpms");

    // then
    // the request should have been redirected to Tasklist
    assertThat(con.getURL().toString()).isEqualTo("http://localhost:" + port + "/eximeebpms/app/tasklist/default/");
  }
}
